...
async function submitRegistration(e){
  ...
  try{
    ...
    const debaters=rows.map(row=>({team_id:teamId,full_name:row.querySelector('[name^="debater_name_"]').value,email:row.querySelector('[name^="debater_email_"]').value||null,role:row.querySelector('select').value,consent:$('#consent').checked}));
    ...
    form.reset();$('#debaters').innerHTML='';addDebater();notify('✓ Inscripción enviada. Quedará pendiente de revisión.');await loadStats();
  }catch(err){console.error(err);notify('No se pudo enviar la inscripción: '+(err.message||'error desconocido'),'error')}
...
async function loadStats(){
  ...
  $('#teamCount').textContent=data?.team_count??0;$('#debaterCount').textContent=data?.debater_count??0;
...
async function loadPublicTeams(){
  ...
  if(tError){grid.innerHTML='<div class="empty">No se pudieron cargar los participantes.</div>';return}
...
async function loadAdmin(){
  ...
  const [{data:teams,error:tErr},{count:debaterCount,error:dErr},{count:pendingCount,error:rErr}]=await Promise.all([
    db.from('esmeralda_teams').select('id,event_id,team_name,school_name,district,status,created_at').eq('event_id',event?.id||'00000000-0000-0000-0000-000000000000').order('created_at',{ascending:false}),
  ...
  if(tErr)console.error(tErr);if(dErr)console.error(dErr);if(rErr)console.error(rErr);
  $('#adminTeams').textContent=teams?.length??0;$('#adminDebaters').textContent=debaterCount??0;$('#adminPending').textContent=pendingCount??0;
  await renderAdmin(teams||[]);
}
...
async function renderAdmin(teams=[]){
  ...
  if(currentPanel==='overview'){
    ...
    panel.innerHTML=adminShell('Resumen',`<div class="admin-grid"><div class="admin-card"><span>Equipos</span><strong>${teams.length}</strong></div><div class="admin-card"><span>Pendientes</span><strong>${pending}</strong></div><div class="admin-card"><span>Aprobados</span><strong>${approved}</strong></div><div class="admin-card"><span>Rechazados</span><strong>${rejected}</strong></div></div><div class="empty">El flujo principal es: inscripción → revisión → aprobación → publicación → organización de rondas → enfrentamientos → evaluación → resultados.</div>`);return;
  }
...
async function setTeamStatus(id,status){
  ...
  await loadAdmin();await loadPublicTeams();await loadStats();
}

async function showTeamDetails(id){
  const [{data:t},{data:coach},{data:debaters}]=await Promise.all([
  ...
  if(!t){alert('Equipo no encontrado.');return}
...
async function renderMatches(panel){
  const [{data:rounds},{data:teams}]=await Promise.all([db.from('esmeralda_rounds').select('id,round_number,name').eq('event_id',event.id).order('round_number'),db.from('esmeralda_teams').select('id,team_name').eq('event_id',event.id).eq('status','approved').order('team_name')]);
  const roundIds=(rounds||[]).map(r=>r.id);let matches=[];
  if(roundIds.length){const {data}=await db.from('esmeralda_matches').select('id,round_id,team_a_id,team_b_id,room,status,winner_team_id').in('round_id',roundIds).order('created_at');matches=data||[]}
...
async function renderAnnouncements(panel){
  ...
  $('#announcementForm').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);const {error}=await db.from('esmeralda_announcements').insert({event_id:event.id,title:fd.get('title'),content:fd.get('content'),published:fd.get('published')==='true'});if(error)alert(error.message);else await renderAnnouncements(panel)};
}
...
async function handleAdminAction(target){
  ...
  if(action==='approve'){document.getElementById('teamDetailDialog')?.close();await setTeamStatus(id,'approved');return}
  if(action==='reject'){if(confirm('¿Confirmas que quieres rechazar este equipo?')){document.getElementById('teamDetailDialog')?.close();await setTeamStatus(id,'rejected')}return}
