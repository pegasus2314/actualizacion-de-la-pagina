...
async function loadEvent(){
  ...
  if(error) console.error(error); event = data;
  if(event){ $('#registrationForm').dataset.eventId = event.id; }
}

function addDebater(){
  const id = debaterIndex++;
  const row = document.createElement('div'); row.className='debater-row'; row.dataset.id=id;
  row.innerHTML=`<input name="debater_name_${id}" required placeholder="Nombre completo" aria-label="Nombre del integrante"/><input name="debater_email_${id}" type="email" placeholder="Correo"/><select name="debater_role_${id}"><option value="captain">Capitán/a</option><option value="debater" selected>Debatiente</option><option value="alternate">Suplente</option><option value="coach">Coach</option></select><button type="button" class="remove" aria-label="Eliminar integrante">×</button>`;
  row.querySelector('.remove').onclick=()=>row.remove(); $('#debaters').appendChild(row);
}

async function submitRegistration(e){
  e.preventDefault();
  const form=e.currentTarget, msg=$('#formMessage'), btn=form.querySelector('button[type=submit]');
  msg.textContent='Enviando inscripción…'; btn.disabled=true;
  try{
    if(!event) throw new Error('El evento no está disponible para inscripción.');
    const fd=new FormData(form);
    const rows=[...document.querySelectorAll('.debater-row')];
    ...
    const debaters=rows.map((r,i)=>({team_id:team.id,full_name:r.querySelector(`[name^="debater_name_"]`).value,email:r.querySelector(`[name^="debater_email_"]`).value||null,role:r.querySelector('select').value,consent:$('#consent').checked}));
    const {error:dError}=await db.from('esmeralda_debaters').insert(debaters); if(dError) throw dError;
    const {error:rError}=await db.from('esmeralda_registrations').insert({event_id:event.id,team_id:team.id,registration_type:'team',status:'pending',payload:{team_name:teamPayload.team_name,school_name:teamPayload.school_name,contact_email:teamPayload.contact_email}}); if(rError) throw rError;
    form.reset(); $('#debaters').innerHTML=''; addDebater(); msg.textContent='✓ Inscripción enviada correctamente. La organización revisará tu solicitud.'; msg.style.color='var(--mint)'; loadStats();
  }catch(err){console.error(err); msg.textContent='No se pudo enviar la inscripción: '+(err.message||'error desconocido'); msg.style.color='var(--danger)';}
  finally{btn.disabled=false;}
}

async function loadStats(){
  const {count:teams}=await db.from('esmeralda_teams').select('*',{count:'exact',head:true});
  const {count:debaters}=await db.from('esmeralda_debaters').select('*',{count:'exact',head:true});
  $('#teamCount').textContent=teams ?? 0; $('#debaterCount').textContent=debaters ?? 0;
}
...
async function loadPublicTeams(){
  ...
  // Participant directory intentionally exposes only non-sensitive team identity.
  const {data:teams,error:tError}=await db.from('esmeralda_teams').select('team_name,school_name,district,status').eq('event_id',data.id).eq('status','approved').order('team_name');
...
async function loadAdmin(){
  ...
  $('#adminTeams').textContent=teams?.length??0; $('#adminDebaters').textContent=debaters?.length??0; $('#adminPending').textContent=regs?.length??0;
...
$('#addDebater').onclick=addDebater; $('#registrationForm').addEventListener('submit',submitRegistration); $('#adminBtn').onclick=()=>$('#loginDialog').showModal(); $('#closeLogin').onclick=()=>$('#loginDialog').close(); $('#loginForm').addEventListener('submit',adminLogin);
addDebater();
loadEvent(); loadStats(); loadPublicTeams();
