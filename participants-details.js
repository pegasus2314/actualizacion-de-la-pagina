(()=>{
  ...
  function renderShell(grid,data){
    ...
    grid.innerHTML=`<div class="participants-switcher" role="tablist"><button type="button" class="participants-tab active" data-participants-tab="people">Participantes <span>${data.length}</span></button><button type="button" class="participants-tab" data-participants-tab="teams">Equipos <span>${groups.size}</span></button></div><div class="participants-pane" data-participants-pane="people"></div><div class="participants-pane hidden" data-participants-pane="teams"></div>`;
    const teamsPane=grid.querySelector('[data-participants-pane="teams"]');
    teamsPane.innerHTML=groups.size?[...groups.values()].map(team=>`<article class="participant-detail-team"><header class="participant-detail-team-head"><div><span class="eyebrow">EQUIPO APROBADO</span><h3>${esc(team.team_name)}</h3><p>${esc(team.school_name||'Centro educativo')}${team.district?' · '+esc(team.district):''}</p></div><span class="member-count">${team.members.length} ${team.members.length===1?'participante':'participantes'}</span></header><div class="participant-detail-members">${team.members.map((p,i)=>memberRow(p,i+1)).join('')}</div></article>`).join(''):'<div class="empty">Aún no hay equipos aprobados.</div>';
