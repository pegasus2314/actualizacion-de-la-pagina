(()=>{
  ...
  // El QR es operativo: solo administración y staff autorizado lo pueden usar.
  // Los roles de debatiente nunca reciben el botón.
  ...
  function renderShell(grid,data){
    ...
    peoplePane.innerHTML=data.length?`
      <div class="participants-toolbar"><div><strong>Participantes registrados</strong><small>Selecciona una persona para consultar su acreditación.</small></div><input class="participants-search" type="search" placeholder="Buscar participante..." aria-label="Buscar participante"></div>
  ...
  function qrButton(p){
    return canSeeQr?`<button class="participant-detail-qr" type="button" data-public-qr="${esc(p.id)}">QR</button>`:'';
  }
