...
async function load(){
 ...
 if(type==='participant'){
  ...
  const base=p[0];
  render({full_name:base.full_name,role:base.role,school_name:base.school_name,district:base.district,status:'pending'});
  const {data:user}=await db.auth.getUser();
  if(user?.user){const {data:priv}=await db.from('esmeralda_debaters').select('*').eq('id',id).maybeSingle();if(priv)render({...base,...priv,first_name:priv.first_name||base.full_name.split(' ')[0],last_name:priv.last_name||base.full_name.split(' ').slice(1).join(' '),status:priv.accreditation_status},true)}
  return;
 }
