(()=>{
  ...
  const ROLE_LABELS={
    admin:'Administrador',
    subsecretario_acreditacion:'Subsecretario de Acreditación',
    debatiente:'Debatiente'
  };

  const PANEL_RULES={
    admin:['overview','teams','rounds','matches','announcements','accreditation','logistics'],
    subsecretario_acreditacion:['overview','accreditation','logistics'],
    debatiente:[]
  };
  ...
  async function getRole(){
    ...
    if(error){console.error('role-access',error);return null}
  ...
  function applyRole(){
    ...
    const navItems=[...document.querySelectorAll('#adminNav a,#adminNav button,.admin-nav a,.admin-nav button,[data-panel]')];
    navItems.forEach(el=>{
    ...
    if(!badge && document.querySelector('#adminWelcome')){
      badge=document.createElement('span');badge.id='trdRoleBadge';badge.className='trd-role-badge';
      document.querySelector('#adminWelcome')?.parentElement?.appendChild(badge);
    }
  ...
  db.auth.onAuthStateChange(()=>setTimeout(refresh,80));
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',refresh);else refresh();

  const style=document.createElement('style');
  style.textContent=`
    .role-hidden{display:none!important}
    .trd-role-badge{display:inline-flex;align-items:center;margin-left:9px;padding:5px 9px;border:1px solid rgba(25,220,229,.2);border-radius:999px;background:rgba(25,220,229,.07);color:#7de7ec;font-size:10px;font-weight:800}
  `;
